...
def calculate_garman_klass_volatility(df: pd.DataFrame, window: int = 14) -> pd.Series:
    """Garman-Klass Volatility with robust epsilon bounds against zero-range candles."""
    high = np.maximum(df["high"], df["low"] + 1e-6)
    low = df["low"]
    open_ = np.maximum(df["open"], 1e-6)
    close = np.maximum(df["close"], 1e-6)
...
def calculate_bollinger_bands(series: pd.Series, period: int = 20, num_std: float = 2.0) -> Tuple[pd.Series, pd.Series, pd.Series, pd.Series]:
    ...
    std = series.rolling(window=period).std().fillna(1e-6)
    upper = mid + num_std * std
    lower = mid - num_std * std
    z_score = (series - mid) / (std + 1e-9)
    return upper, lower, mid, z_score.fillna(0.0)

def build_feature_matrix(df: pd.DataFrame) -> pd.DataFrame:
    features = pd.DataFrame(index=df.index)
    features["log_ret"] = np.log(df["close"] / df["close"].shift(1)).fillna(0.0)
    features["gk_vol"] = calculate_garman_klass_volatility(df)
    features["rsi"] = (calculate_rsi(df["close"]) / 100.0 - 0.5)
    ...
    features["bollinger_z"] = bz.clip(-4.0, 4.0)
    v_mean = df["volume"].rolling(20).mean().bfill().fillna(1.0)
    features["vol_ratio"] = ((df["volume"] / v_mean) - 1.0).clip(-3.0, 3.0)
